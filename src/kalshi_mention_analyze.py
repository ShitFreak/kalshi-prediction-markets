...
for ev, ws in by_ev.items():
    ...
    t = min(yj); q = t - 600                      # 10 min before first word -> YES
    for w in ws:
        seq = w['seq']
        if not seq: continue
        if seq[0][0] > q: cov_drop += 1; continue  # market opened after t-10min
        px = price_at(seq, q)
        if px is None or not (0.0 < px < 1.0): continue
        points.append((px, w['win'], ev, w['vol'])); contrib_events.add(ev)

print(f"events: {len(by_ev)} | anchorable: {len(by_ev)-nojump} | contributing: {len(contrib_events)}")
print(f"word-points: {len(points)} | dropped (opened after t-10min): {cov_drop} | no-jump events: {nojump}")
...
print(f"\nbias slope b = {beta:+.3f} | alpha {alpha:+.3f} | mean px {p.mean()*100:.0f}c | avg ret {(y-p).mean()*100:+.1f}%")
...
ax.plot(xs, ws_, '-o', color='#6a1b9a', lw=2.5, ms=7,
...
ax.set_title(f"Politician MENTION markets — pre-speech (t-10min) odds vs actual\n"
             f"24h window | bias slope b = {beta:+.3f} | dear>75c ret {dr*100:+.1f}%  (below 45° = overpriced)",weight='bold')
ax.set_xlabel("pre-speech price = implied chance the word is said")
ax.set_ylabel("actual rate the word was said")
ax.set_xlim(0,1); ax.set_ylim(0,1); ax.legend(loc='upper left'); ax.grid(alpha=.3)
out = r"C:\Users\Riyan\Downloads\kalshi_mention_calibration.png"
